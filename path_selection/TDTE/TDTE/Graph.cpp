...
/* added */
Graph::Graph(int nodeNum, vector<pair<int, int>> dirEdge, vector<double> edgeWeight) {
	//1. Reset the members of the class
	clear();
	//2 The first is the number of vertices of the graph
	m_nVertexNum = nodeNum;

	//3 The following lines, each line contains a directed edge in the graph:
	///   the id of starting point, the id of ending point, the weight of the edge. 
	///   These values are separated by 'white space'. 
	int start_vertex, end_vertex;
	double edge_weight;
	int vertex_id = 0;
	int dirEdgeNum = dirEdge.size();
	for (int i = 0; i < dirEdgeNum; i++) {
		///3.1 get the vertices
		start_vertex = dirEdge[i].first;
		if (start_vertex == -1)
		{
			break;
		}
		end_vertex = dirEdge[i].second;;
		edge_weight = edgeWeight[i];

		///3.2 construct the vertices
		BaseVertex* start_vertex_pt = get_vertex(start_vertex);
		BaseVertex* end_vertex_pt = get_vertex(end_vertex);

		///3.3 add the edge weight
		...
		///3.4 update the fan-in or fan-out variables
		//// Fan-in
